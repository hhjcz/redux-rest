...
import { selectResourceFn } from '../index'
...
describe('resources selectors', () => {
  ...
  it('selects resource', () => {
    const resource1State = selectResourceFn(state => state)('resource1')(state)
    expect(resource1State.foo1).to.deep.equal('foo1')
  })

  it('selects with inserted items', () => {
    const resource3 = selectResourceFn(state => state)('resource3')(state)
